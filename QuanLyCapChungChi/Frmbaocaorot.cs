...
{
    public partial class Frmbaocaorot : Form
    {
        private readonly object _maKhoaThi;
        public Frmbaocaorot(object maKhoaThi)
        {
        ...
        private void Frmbaocaorot_Load(object sender, EventArgs e)
        {
            ...
            string sql = @"
                ...
                WHERE kqt.MAKHOATHI = @MaKhoaThi -- Lọc theo khóa thi được chọn
                  AND ( -- Thêm điều kiện lọc chỉ những người ĐẬU
                        (kt.TENKHOATHI LIKE N'%Tin học%' AND kqt.DIEMSO < 5.0 OR kqt.DIEMSO2 < 5.0)
                        OR
                        (kt.TENKHOATHI LIKE N'%Tiếng Anh%' AND kqt.DIEMSO < 5.0 OR kqt.DIEMSO2 < 5.0 OR kqt.DIEMSO3 < 5.0)
                      )
